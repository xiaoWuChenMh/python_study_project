...
def image_channel(image):
    ...
    Args:
        image (np.ndarray):
      检查图像数据中的通道数，如果是三通道图像，返回为通道数；否则返回 0。
    Returns:
...
def get_color(image, area):
    ...
    # 计算整个图像或图像特定区域的颜色均值（计算多通道图像的每个通道的均值）
    color = cv2.mean(temp)
    # 返回这个均值数组的前三个元素，它们分别代表蓝色、绿色和红色通道的均值。
    return color[:3]
...
def get_bbox(image, threshold=0):
    ...
    if image_channel(image) == 3:
        # 果图像具有三个通道（RGB），则通过沿第三个轴（颜色通道）取最大值，将其合并为单个通道的灰度图像。
        image = np.max(image, axis=2)
    # 根据提供的阈值获取图像中亮度最高的区域的边界框的 x 和 y 坐标
